...
## 🧠 思维方式铁律（2026-04-22 17:00 主人严厉纠正）

**主人原话**："你应该思考如何升级、如何正向修改目前的问题。
              夜间亏就判定所有夜间都亏损，你的思考方式不专业。"

### 专业 vs 业余的核心区别

| 业余（禁止）| 专业（必做）|
|---|---|
| 发现问题 → 关闭/回退 | 发现问题 → **深挖原因 + 找赚钱升级** |
| 样本 100 笔就下结论 | 要 30-90 天数据 + **显著性检验** |
| 防御思维（缩仓/加 gate） | 进攻思维（改信号/换策略/加 alpha）|
| 凭感觉"看起来不好" | 凭**量化证据**（IC / Sharpe / p-value）|
| 过拟合特定时段 | 问"**为什么**"找结构性原因 |

### 案例：CST 00-03 时段亏损分析
**业余做法**（我犯的错）：建议 DISABLED_UTC_HOURS = [16,17,19]，关闭该时段。
**专业做法**：
1. 样本足够了吗？（19 笔不够 —— 需 200+ 笔）
2. 统计显著吗？（t-test p-value < 0.05？）
3. **原因分析**：那时段是美国早盘 / 欧洲尾盘，为什么对 grid 不利？
   - 流动性？波动率？方向性？特定经济数据？
4. **升级方案**（非关闭）：
   - 若波动高 → 那时段自动改用 trend_follow（震荡策略失效）
   - 若流动性差 → 那时段加宽 spacing（吃更大价差）
   - 若方向单边 → 那时段把 direction gate 阈值放严

**只有在升级方案都尝试失败后**，才考虑"关闭"作为最后兜底。

### 铁律
- **任何"关闭/暂停/缩仓"建议必须配一份"升级失败后的兜底"**
- 先做升级尝试，而不是第一反应就防御
- 防御 = 缩小机会；升级 = 找到机会

## 📋 2026-04-22 17:00 起 每轮必做任务（主人 B 方案调试期）

主人选方案 B：继续跑 + 边跑边升级 + 到明天上午要彻底答复。
**你（daemon）承担 24/7 自动化** —— 不再依赖主人 Mac 对话端的 ScheduleWakeup。

### A. 实盘守护（每轮必做）
1. 查账户 + 持仓 + 近 30min/1h/2h/今日 PnL
2. 比对 Circuit Breaker 阈值（WARN/PAUSE/HALT/KILL）
3. 若临近 KILL（今日亏 > $6.0）→ 立刻邮件 [紧急]
4. 若 strategy 进程挂了 → log 记录（systemd 会自动拉起）

### B. 数据采集进度（每轮必做）
1. `wc -l data/signal_snapshots.jsonl`（目标 24h 内 > 500 条）
2. `wc -l data/fill_quality.jsonl`（目标 > 20 条）
3. `cat data/perf_eval.json | jq .sharpe_annualized`（Sharpe 变化追踪）

### C. 白皮书填充（每轮推进 1-2 章）
编辑 `data/whitepaper_draft.md`，按数据充实各章节：
- 第 1 章因子多少：当 IC 数据 > 500 样本时填完整
- 第 2 章 IC 排名：运行 signal_attribution --analyze 获取 IC
- 第 3 章下单决策：详细梳理 13 道 gate 的命中率
- 第 4 章方向判断：统计 direction=long/short 各自 PnL
- 第 5 章 OKX 策略：补充 rate limit / margin / leverage 实测
- 第 6 章手续费：已有基础，补充 maker/taker 比例历史变化
- 第 7 章策略多少：评估 grid / trend / mean_rev 各自贡献
- 第 8 章 TOP 10：持续更新发现的新问题
- 第 9 章升级路线：按 ROI 排序 + 每条配"升级方案"而非"关闭"
- 第 10 章结论：给主人的明确建议

### D. 升级思考（每轮至少 1 个）
**主人铁律：正向思维**
每轮发现任何问题，必做：
1. 写下问题（data/upgrade_ideas.md）
2. 问"为什么"（根因）
3. 想 3 个**升级方案**（不是关闭）
4. 选 ROI 最高的 1 个
5. 评估：小改（立刻改 + commit）/ 大改（写方案进 roadmap）

### E. 明天上午交付检查
每轮都问自己：白皮书是否已填够 70%？IC 数据够吗？TOP 10 升级方案都有了吗？
缺什么 → 本轮补。

### F. 睡眠节奏
- 熔断临近 / 持仓浮亏 > $1 → next_sleep=120-180
- 正常稳定 → next_sleep=300-600
- 深夜稳定 → next_sleep=600-900（省 Max 配额）
