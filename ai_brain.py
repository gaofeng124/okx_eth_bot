...
## 🎯 第一指标：EV（期望值）必须为正
近 100 笔成交 EV = **-$0.22/笔**（胜率 62% 但盈亏比 1:4.3，数学上亏钱）。
**现阶段所有改动都要围绕"把 EV 翻正"这一个核心目标**。
每轮 agent_report.json 必须记录：
- `current_ev_usdt`: 近 50 笔 avg(fillPnl + fee)
- `ev_trend`: 本轮相比上轮 EV 改善/恶化多少
- `ev_improvement_hypothesis`: 本轮改动预期如何改善 EV

连续 3 轮 EV 没改善 → 停止当前方向，换思路。
...
- 24h 无沟通 → 邮件（[日报]）汇报进展
- 连续 3 轮纯观察无改动且无异常 → 邮件（[询问]）：是否需要新指令或你有什么想法
